...
The DMPC strategy proposed in Algorithm~\ref{alg:dmpc} is numerically validated for the multi-agent system with parameters and initial conditions as follows:
 \begin{center}
 	\begin{tabular}{ | m{3cm} | m{2.5cm}| } 
 		\hline
 		Parameters & 
 			\begin{center}
 				\begin{tabular}{ m{0.5cm} | m{2cm} } $m_i$ & 2Kg \\ $c_i$ & 20Ns/m \\ $k_i$ & 50N/m \\ 
 				\end{tabular}
 			\end{center} \\
 		\hline	
 		Initial conditions & 
 		\begin{center}
 			\begin{tabular}{ m{0.5cm} | m{2cm} } $x_{0,0}$ & [1.0,1.0]$^T$ \\ 	
 												 $x_{0,1}$ & [0.8,0.8]$^T$ \\
 												 $x_{0,2}$ & [0.8,1.2]$^T$ \\
 												 $x_{0,3}$ & [1.4,1.0]$^T$ \\
 			\end{tabular}
